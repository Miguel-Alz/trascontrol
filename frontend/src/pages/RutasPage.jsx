...
export default function RutasPage() {
  ...
  const [formData, setFormData] = useState({ nombre: '', numero: '', origen: '', destino: '', descripcion: '', activo: true });
  ...
  const openModal = (item = null) => {
    if (item) {
      setEditing(item);
      setFormData({ nombre: item.nombre, numero: item.numero || '', origen: item.origen || '', destino: item.destino || '', descripcion: item.descripcion || '', activo: item.activo });
    } else {
      setEditing(null);
      setFormData({ nombre: '', numero: '', origen: '', destino: '', descripcion: '', activo: true });
    }
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await rutasAPI.update(editing.id, formData);
        toast.success('Éxito', 'Ruta actualizada');
      } else {
        await rutasAPI.create(formData);
        toast.success('Éxito', 'Ruta creada');
      }
      closeModal();
      loadData();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.success('Éxito', 'Ruta eliminada');
      loadData();
    } catch (error) {
      toast.error('Error', error.message);
    }
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 className="text-xl font-semibold">🛣️ Gestión de Rutas</h2>
        <div className="flex gap-2">
          <Button variant="secondary" onClick={() => setShowFilters(!showFilters)}>
            <Filter className="w-4 h-4" /> Filtros
          </Button>
          <Button onClick={() => openModal()}><Plus className="w-4 h-4" /> Nueva Ruta</Button>
        </div>
      </div>
        ...
        <div className="glass-card rounded-xl p-4 animate-slideIn">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input 
              ...
              placeholder="Ej: Centro, Terminal..." 
            />
          </div>
      ...
      <div className="glass-card rounded-xl overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700/50 bg-slate-800/30">
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-400 uppercase">Número</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-400 uppercase">Nombre</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-400 uppercase">Origen</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-400 uppercase">Destino</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan="6" className="px-6 py-8 text-center text-slate-500">No hay rutas registradas</td></tr>
              ) : (
                rutas.map((item) => (
                  <tr key={item.id} className="border-b border-slate-700/30 hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-400">{item.origen || '-'}</td>
                    <td className="px-6 py-4 text-slate-400">{item.destino || '-'}</td>
        ...
        <div className="px-6 py-4 border-t border-slate-700/50 flex items-center justify-between text-sm text-slate-500">
          <span>Mostrando {rutas.length} de {pagination.total} rutas</span>
          <div className="flex gap-2">
            <Button 
              variant="ghost" 
              size="sm" 
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editing ? 'Editar Ruta' : 'Nueva Ruta'}>
        <form onSubmit={handleSubmit}>
          <Modal.Body>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <Input label="Número" value={formData.numero} onChange={(e) => setFormData({ ...formData, numero: e.target.value })} placeholder="Ej: 101" />
                <Input label="Nombre" value={formData.nombre} onChange={(e) => setFormData({ ...formData, nombre: e.target.value })} placeholder="Ej: Ruta Centro" required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Input label="Origen" value={formData.origen} onChange={(e) => setFormData({ ...formData, origen: e.target.value })} placeholder="Ej: Terminal" />
                <Input label="Destino" value={formData.destino} onChange={(e) => setFormData({ ...formData, destino: e.target.value })} placeholder="Ej: Centro" />
              </div>
          ...
          <Modal.Footer>
            ...
            <Button type="submit" isLoading={isSaving}>{editing ? 'Guardar Cambios' : 'Crear Ruta'}</Button>
          </Modal.Footer>
        </form>
      </Modal>
    </div>
